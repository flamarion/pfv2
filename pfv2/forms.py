...
SUBMIT_FIELD_RENDER = {"class": "btn btn-primary"}

SELECT_FIELD_RENDER = {"class": "form-control form-control-sm"}
...
DATE_FIELD_RENDER = {"class": "form-control form-control-sm", "type": "date", "id": "dateForm"}


class RegistrationForm(FlaskForm):
    username = StringField(
        "Username", validators=[DataRequired()], render_kw={**STRING_FIELD_RENDER, "placeholder": "Username"}
    )
    email = StringField(
        "Email",
        validators=[DataRequired(), Email()],
        render_kw={**STRING_FIELD_RENDER, "placeholder": "user@example.com"},
    )
    password = PasswordField(
        "Password", validators=[DataRequired()], render_kw={**STRING_FIELD_RENDER, "placeholder": "Password"}
    )
    password2 = PasswordField(
        "Repeat Password", validators=[DataRequired(), EqualTo("password")], render_kw={**STRING_FIELD_RENDER}
    )
...
class AddAccountTypeForm(FlaskForm):
    account_type = StringField(
        label="Account Type", validators=[DataRequired()], render_kw={**STRING_FIELD_RENDER, "placeholder": "Checking"}
    )
...
class AddAccountForm(FlaskForm):
    account_name = StringField(
        label="Account Name", validators=[DataRequired()], render_kw={**STRING_FIELD_RENDER, "placeholder": "ING"}
    )
    ...
    initial_balance = DecimalField(
        ...
        render_kw={"type": "number", "step": "0.01", "class": "form-control", "placeholder": "50.00"},
        places=2,
        rounding=None,
    )
...
class AddBudgetForm(FlaskForm):
    budget_name = StringField(
        label="Budget Name", validators=[DataRequired()], render_kw={**STRING_FIELD_RENDER, "placeholder": "Leisure"}
    )
    budget_month = DateField(label="Month", validators=[DataRequired()], render_kw=MONTH_FIELD_RENDER, format="%m-%Y")
    budget_total = DecimalField(
        label="Value",
        validators=[DataRequired()],
        render_kw={"type": "number", "step": "0.01", "class": "form-control", "placeholder": "50.00"},
        places=2,
...
class AddCatetoryForm(FlaskForm):
    category_name = StringField(
        label="Category Name", validators=[DataRequired()], render_kw={**STRING_FIELD_RENDER, "placeholder": "Gasoline"}
    )
...
class TransactionForm(FlaskForm):
    tr_type = SelectField(
        ...
        render_kw=SELECT_FIELD_RENDER,
    )
    tr_desc = StringField(
        label="Description", validators=[DataRequired()], render_kw={**STRING_FIELD_RENDER, "placeholder": "Kruidwat"}
    )
    tr_date = DateField(label="Date", validators=[DataRequired()], render_kw=DATE_FIELD_RENDER)
    tr_value = DecimalField(
        label="Value",
        validators=[DataRequired()],
        render_kw={"type": "number", "step": "0.01", "class": "form-control", "placeholder": "50.00"},
        places=2,
    ...
    tr_budget = SelectField(label="Budget", validators=[DataRequired()], render_kw=SELECT_FIELD_RENDER)

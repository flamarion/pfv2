...
def frontpage(op=None, id=None):
    if request.method == "POST":
        ...
        tr_budget = request.form["tr_budget"]
        ...
        if op == "edit":
            ...
            transaction.budget_id = tr_budget
            ...
            # Update Account and Budget balance
            account = Account.query.filter_by(id=tr_account).filter_by(owner_id=current_user.id).first()
            budget = Budget.query.filter_by(id=tr_budget).filter_by(owner_id=current_user.id).first()
            ...
            if tr_type == "expense":
                budget.balance = budget_balance - tr_value
                account.balance = account_balance - tr_value
        ...
        # Edit transaction
            ...
            categories = [(i.id, i.name) for i in Category.query.filter_by(owner_id=current_user.id).all()]
            form.tr_budget.choices = [(i.id, i.name) for i in budgets]
            form.tr_account.choices = account_names
            form.tr_category.choices = [("", "---")] + categories
            return render_template(
                "front_page.html", form=form, accounts=accounts, transactions=transactions, budgets=budgets
            )

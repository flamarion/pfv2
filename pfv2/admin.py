...
def adm_accounts(op=None, id=None, accounts=None):
    if request.method == 'POST':
        ...
        if op == "add":
            ...
            return render_template('add_account.html', form=form, accounts=Account.query.filter_by(owner_id=current_user.id).all())

        # Add Account Type
        elif op == "addtype":
            form = AddAccountTypeForm()
            return render_template('add_account_type.html', form=form, account_types=AccountType.query.filter_by(owner_id=current_user.id).all())
    ...
    if request.method == 'POST':
        ...
        # Create new budget
            ...
            if op == 'addbudget':
                ...
                new_budget = Budget(name=budget_name,
                                    year=budget_year,
                                    month=budget_month,
                                    total=budget_total,
        ...
        if op == "addbudget":
            form = AddBudgetForm()
            return render_template('add_budget.html', form=form, budgets=Budget.query.filter_by(owner_id=current_user.id).all())
    ...
    if request.method == 'POST':
        ...
        # Create new Category
            ...
            new_category = Category(name=category_name,
                                    budget_id=category_budget,
                                    owner_id=category_owner)
            try:
